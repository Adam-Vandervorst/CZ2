...
class ExprMapEngine[V]:
  private def prefix[W](i: Long)(res: ExprMap[W]): ExprMap[ExprMap[W]] = if res.isEmpty then ExprMap() else
    ExprMap(EM(ExprMap(if res.em.apps.em eq null then null else EM(
