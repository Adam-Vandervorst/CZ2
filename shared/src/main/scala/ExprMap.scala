...
case class EM[V](apps: ExprMap[ExprMap[V]],
  ...
  final def updated(e: Expr, v: V): EM[V] =
    val c = this.copy()
    c.update(e, v)
    c
  ...
  final def getUnsafe(e: Expr): V = e match
    case Var(i) => vars(i)
    case App(f, a) => apps.getUnsafe(f).getUnsafe(a)

  final def get(e: Expr): Option[V] = e match
    case Var(i) => vars.get(i)
    case App(f, a) => apps.get(f).flatMap(_.get(a))

  final def getAt(ins: IterableOnce[Option[Int]]): Either[EM[_], V] =
    var loc: EM[_] = this
    var apps = 0
    var it = ins.iterator
  ...
  final def keys: Iterable[Expr] =
    val ks = mutable.ArrayDeque.empty[Expr]
    foreachKey(ks.append)
    ks

  final def items: Iterable[(Expr, V)] =
  ...
  final def union(that: EM[V]): EM[V] =
    EM(
  ...
  final def map[W](f: V => W): EM[W] = EM(
    apps.map(_.map(f)),
    vars.mapValuesNow(f)
  )

  final def collect[W](pf: PartialFunction[V, W]): EM[W] =
    EM(
  ...
  final def indiscriminateMatching(e: Expr): ExprMap[V] = e match
  ...
  final def indiscriminateReverseMatching(e: Expr): ExprMap[V] = e match
  ...
  final def indiscriminateBidirectionalMatching(e: Expr): ExprMap[V] = e match
    case Var(i) if i > 0 =>
...
//      println(s"stepping into apps removing symbols from ${this.prettyStructuredSet()}")
  ...
  final def transform(pattern: Expr, template: Expr): ExprMap[V] =
  ...
  final def transformMatches(pattern: Expr, template: Expr): ExprMap[V] =
    val possible = indiscriminateBidirectionalMatching(pattern)
  ...
  final def foreachKey(func: Expr => Unit): Unit =
  ...
  final def foreach(f: V => Unit): Unit =
    vars.foreachValue(f)
    apps.foreach(_.foreach(f))

  final def size: Int = foldRight(0)((_, c) => c + 1)
  ...
  final def isEmpty: Boolean = vars.isEmpty && apps.isEmpty
end EM
...
case class ExprMap[V](var em: EM[V] = null) extends EMImpl[V, ExprMap]:
  final def copy(): ExprMap[V] = if em eq null then ExprMap() else ExprMap(em.copy())
  final def contains(e: Expr): Boolean = if em eq null then false else em.contains(e)
  inline def getUnsafe(e: Expr): V = em.getUnsafe(e)
  final def get(e: Expr): Option[V] = if em eq null then None else em.get(e)
  final def getAt(ins: IterableOnce[Option[Int]]): Either[EM[_], V] = if em eq null then ins.iterator.nextOption().fold(Left(null)){ ins => throw RuntimeException(s"tried to execute ${ins} on empty") } else em.getAt(ins)
  final def updated(e: Expr, v: V): ExprMap[V] = ExprMap(if em eq null then EM.single(e, v) else em.updated(e, v))
  ...
  final def updateWith(e: Expr)(f: Option[V] => Option[V]): Unit = if em eq null then f(None).foreach(v => em = EM.single(e, v)) else em.updateWith(e)(f)
  final def remove(e: Expr): Option[V] = if em eq null then None else em.remove(e)
  final def keys: Iterable[Expr] = if em eq null then Iterable.empty else em.keys
  final def values: Iterable[V] = if em eq null then Iterable.empty else em.values
  final def items: Iterable[(Expr, V)] = if em eq null then Iterable.empty else em.items
  ...
  final def foreachKey(f: Expr => Unit): Unit = if em ne null then em.foreachKey(f)
  final def foreachItem(f: (Expr, V) => Unit): Unit = if em ne null then em.foreachItem(f)
  final def foreach(f: V => Unit): Unit = if em ne null then em.foreach(f)
  final def map[W](f: V => W): ExprMap[W] = ExprMap(if em eq null then null else em.map(f))
  final def collect[W](pf: PartialFunction[V, W]): ExprMap[W] = ExprMap(if em eq null then null else em.collect(pf))
  final def indiscriminateMatching(e: Expr): ExprMap[V] = if em eq null then ExprMap() else em.indiscriminateMatching(e)
  final def indiscriminateReverseMatching(e: Expr): ExprMap[V] = if em eq null then ExprMap() else em.indiscriminateReverseMatching(e)
  final def indiscriminateBidirectionalMatching(e: Expr): ExprMap[V] = if em eq null then ExprMap() else em.indiscriminateBidirectionalMatching(e)
  ...
  final def execute(instrs: IterableOnce[Instr], debug: 0 | 1 | 2 = 0): ExprMap[V] = new ExprMapEngine[V].execute(this, instrs, debug)
  final def transform(pattern: Expr, template: Expr): ExprMap[V] = if em eq null then ExprMap() else em.transform(pattern, template)
  final def transformMatches(pattern: Expr, template: Expr): ExprMap[V] = if em eq null then ExprMap() else em.transformMatches(pattern, template)
  final def flatMap[W](op: (W, W) => W)(f: V => ExprMap[W]): ExprMap[W] = if em eq null then ExprMap() else em.flatMap(op)(f)
  final def foldRight[R](z: R)(op: (V, R) => R): R = if em eq null then z else em.foldRight(z)(op)

  private final def appliedRec[W](fem: ExprMap[W], aem: ExprMap[W]): ExprMap[ExprMap[W]] =
  ...
  final def applied(aem: ExprMap[V]): ExprMap[V] =
    ExprMap(EM(appliedRec(this, aem), VarMap.empty))

  private final def appliedWithRec[W](op: (W, W) => W)(fem: ExprMap[W], aem: ExprMap[W]): ExprMap[ExprMap[W]] =
  ...
  final def appliedWith(op: (V, V) => V)(aem: ExprMap[V]): ExprMap[V] =
    ExprMap(EM(appliedWithRec(op)(this, aem), VarMap.empty))

  final def appliedWithSim[W](op: (W, W) => W)(fem: ExprMap[W])(aem: ExprMap[W]): ExprMap[W] =
    ExprMap.from(fem.items.flatMap((fe, fw) => aem.items.map((ae, aw) => Expr(fe, ae) -> op(fw, aw))))

  final def size: Int = if em eq null then 0 else foldRight(0)((_, c) => c + 1)
  final def prettyStructured(colored: Boolean = true, tree: Boolean = false): String = EMPrettyPrinter.structured(this, colored=colored, tree=tree)
  final def prettyStructuredSet(colored: Boolean = true, tree: Boolean = false): String = EMPrettyPrinter.structuredSet(this, colored=colored, tree=tree)
  final def prettyListing(colored: Boolean = true): String = EMListPrinter.listing(this, colored=colored)
  final def json: String = EMJSONPrinter.structured(this, colored=false)
